...
function HomePage() {
  ...
  useEffect(() => {
    if (heroRef && headerRef && buttonRef) {
      const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            headerRef.current.classList.add(classes.hidden);
            buttonRef.current.classList.remove(classes["hidden-button"]);
          } else {
            headerRef.current.classList.remove(classes.hidden);
            buttonRef.current.classList.add(classes["hidden-button"]);
          }
        });
      });
  ...
  return (
    <>
      ...
      <div ref={heroRef} className={classes.hero}>
        ...
        <a href="#content">
          <button ref={buttonRef} className={classes["nav-button"]}>
            <svg
              viewBox="0 -4.5 20 20"
              version="1.1"
      ...
      <footer className={classes.footer}>Projekt ZSI 2023</footer>
    </>
  );
}
